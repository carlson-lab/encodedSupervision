	...
	def transform_withLikelihood(self,s,data_complex,sess):
		...
		for i in range(nTimes):
			...
			for jj in range(nInner):
				sess.run(self.optimstep_e,feed_dict={self.s:s,self.y_fft:data_batch})
		...
		for jj in range(30):
			sess.run(optimizer,feed_dict={s_:s,y_fft_:data_batch})

		score_batch,ll = sess.run([scores,
							logLikelihood],feed_dict={self.s:s,
							self.y_fft:data_batch})
...
class CSFA(CSFA_base):
	def __init__(self,L,reg=.01,eta=5.0,Q=3,nIter=1000,lr_encoder=1e-4,
					...
					nlayer=1,activationFunction='sigmoid',
					batch_size=100,learnVar=False,varLam=1.0):
		super(CSFA,self).__init__(L,reg=reg,
	...
	def _initialize(self,s,Ns,Nc,Nw):
		...
		###########################
		self.loss = self.NLL + 0.5*self.reg*self.reg_scores + 0.01*self.reg*self.reg_features 
	...
	def fit(self,s,data_complex,monitor=None):
		...
		for i in range(self.nIter):
			...
			_,ll,loglike =sess.run([self.optimstep_l,self.loss,self.NLL],feed_dict={self.s:s,
							self.y_fft:aa2})
			...
			if i%self.printIter == 0:
				...
				nl = self.reg*sess.run(self.NLL,
										feed_dict={self.s:s,
										self.y_fft:aa2})
				print('Iteration %d,Time = %0.1f,Loss = %0.3f,scores=%0.3f,feature=%0.3f,NLL=%0.3f'%(int(i),el,ll,sl,fl,nl))
